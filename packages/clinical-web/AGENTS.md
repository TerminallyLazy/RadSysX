...
- `AITextSessionRequest`/`AITextTurnRequest` mirror the focused `ai_text_routes.py` contracts. `createAITextSession` and `submitAITextTurn` use cookies/no-store/fixed errors. Text sessions return `mode=text`, standard Gemini/NVIDIA/Codex model identity and null audio rates/live URL; never interpret these as a live provider profile.
