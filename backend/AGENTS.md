...
- Keep dependency guidance split: `requirements-clinical.txt` is the clinical base; `requirements-ai.txt` layers the modern Gemini/OpenAI transports, NVIDIA research adapter and DeepAgents/LangGraph desktop runtime on it. `requirements.txt` is the incompatible legacy research set and requires its own environment. Never combine the two agent sets.
