...
- Keep dependency guidance split: `requirements-clinical.txt` is the clinical base; `requirements-ai.txt` layers the modern Gemini/OpenAI and deepagents desktop runtime on it. `requirements.txt` is the incompatible legacy research set and requires its own environment. Never combine the two agent sets.
