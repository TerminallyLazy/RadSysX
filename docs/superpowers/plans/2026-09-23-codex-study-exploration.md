...
- [ ] Build `exploration_helpers.make_task(live, *, allow_tools=True, frames=34)` using the existing actor/session fixture and the Task 1 DTOs. It returns `(service, task, binding)`; helpers fabricate only renderer responses, never bypass production policy.
- [ ] Write a route test that polls the same command twice but allows only one successful claim. Use two renderer epochs and assert the old epoch cannot post a result after replacement:
